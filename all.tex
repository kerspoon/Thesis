...
From the graph in Figure \ref{probacceptable} it is much easier to see that the number of failed contingencies is highly non-linear. These are outliers in the sense of being outside the main grouping of results but not such that should be removed. These are interesting cases where a system operator should know that the system is in a precarious state and should be re-balanced to a new and more stable operating point. In some of the later graphs these have been removed for clarity; it does not mean they are any less important; just that it would not be possible to show other data trends if they were included.
...
The contingencies from this section were generated by selecting each combination of two components as the ones that will fail. This gave a total of 41,616 contingencies, far more than is needed using Monte Carlo sampling. 

Interesting to see if the results are similar to N-1 or show different effects. 
