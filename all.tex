...
The shell script given in Section \ref{sec_prog_use} was run with 1000 base cases and 10,000 unique contingencies.
...
Table \ref{table_results_summary} shows the summary of a 20 random base cases.

The column that is of most interest is the third one. This is the number of unacceptable contingencies divided by the total number. It is this number we use to represent the overall security of that base case. Taking the first row as an example in around 3 million contingencies about 1000 were unacceptable and would need emergency operator action. If the number of unacceptable cases were higher it would mean it was more likely that the system operator would need to perform emergency action.

An intuitive feel for the normal state of system operation can also be gained from this table. In most of the cases there were a number of components out of service; these were almost all generators. This is what was expected from the probabilities these are based upon but by viewing it this way it becomes easier to visualise. Additionally 

\subsubsection{Load Level Forecast}

The load level mostly hovers at around half the maximum output, indeed it is the modal value which between 14 and 16\% of base-cases having a power level of 0.5 times the maximum. The range of powers spanned the entirety of the possible values from 0.35 at the lowest to 1 at the highest. It is interesting to note that the median value was around 0.6 meaning that on average 40\% of installed capacity was unused. This is shown in Table \ref{table_load_levels}. An early Chapter in Figure \ref{aggregateforecast} shows this graphically for many more samples than was used in this Chapter.
...
Interestingly if the forecast was high in the base-case, a contingency may cause the power required to be higher than the power available. A clear example of an adequacy issue. Not very surprising as the system in question is used in a lot of adequacy studies.

\subsubsection{Number of Components on Outage in Base Cases}

The number of components that are out for repair or maintenance in each base case ranges from 0 to 13, the exact frequencies are shown in Figure \ref{outagefrequencies1000} and Table \ref{XXX}. This means that in one base-case 13 separate components were unavailable. This does not take into account the fact that the loss of a single busbar may remove a number of generating units. The modal value was 4 with 5 coming in a close second. The median was also between 4 and 5 meaning, for this system, at any given time one would expect 4 components to be non-functioning.

97.5\% of all failures were of Generating units making up 4123 of the total 4229 components that were on outage. There were 100 line outages in the base cases corresponding to a total of 2.4\% and the final 0.1\% of outages was caused by a fault on a busbar, there were only 4 of these in the 1000 base-cases.
...
\hline \hline
component out of limits &  0.8  &  G71, G6, G35, G72  \\ 
component out of limits &  0.8  &  G24, A10, G34, G89, G70, A12-1, G67, G56, A32-1 \\ 
component out of limits &  0.75 &  G22, B24, G71, G45, G72, G63 \\ 
divergence              &  0.6  &  G74, G90, G13, C5, G47 \\ 
islanded                &  0.85 &  C11, G35, G23, G72 \\ 
component out of limits &  0.9  &  G24, G39, G74, G45, B10, G98 \\ 
component out of limits &  0.85 &  G31, G23, G97, G96, G57, G55, G47, G13, G64 \\ 
component out of limits &  0.4  &  G33, B15, G89, G19, G90 \\ 
component out of limits &  0.7  &  G71, G24, G3, G72 \\ 
islanded                &  0.65 &  G41, C11, G83, G72 \\ 
component out of limits &  0.55 &  G31, G5, C10, G12 \\ 
component out of limits &  0.7  &  G76, B10, G1 \\ 
divergence              &  0.8  &  G57, G66, G56, G90 \\ 
component out of limits &  0.65 &  G6, G5, G67, G99 \\ 
\hline
\end{tabular}\\
\end{table}
...
\todo{
table with selection of 20 MC samples.
}
...
\todo{
introduce in the same way as the base case figures. min/max/mode/median for forecast error, outages, 
}
...
From the graph in Figure \ref{XXX} it is much easier to see that the number of failed contingencies is highly non-linear. Even on a logarithmic scale (Figure \ref{XXX}) a few base cases have a much higher number than the others. These are outliers in the sense of being outside the main grouping of results but not such that should be removed. These are interesting cases where a system operator should know that the system is in a precarious state and should be re-balanced to a new and more stable operating point. In some of the later graphs these have been removed for clarity; it does not mean they are any less important; just that it would not be possible to show other data trends if they were included.

Table \ref{XXX} shows 20 of these cases. Even in that selection the number of unacceptable cases goes up two orders of magnitude. The least secure base case on this run actually had more chance of being un acceptable than not. Clearly not a system that an operator would consider running.
...
\todo{add more to this, refer to both tables. min 0.0002914349 max 0.6130704853 avg 0.0033332323}

\todo{
analyse the types of failures how many were islanding, how many for limit violations.
}
...
exerpt from both sample file and analysis.
average, min, max components on failure (bar chart num components, times seen) take just as failure not combined.
higest lowest and average power as a percentage of the system capability.  (probability density function)

p acceptable: min, max, average. (run twice to see if p acceptable is fairly constant).
...
exerpt from both sample file and analysis.
total number of fail cases.
how many were bus fails, \#line \#gen
see how it compares to a probability list of contingencies n long.
n-1 as a predictor of p(acc)
...
see how it compares to a probability list of contingencies n long.
n-1 as a predictor of p(acc)
is a full n-2 worth anything? (conclusion)
...
what does unscheduleable mean in this instance
which gens have I picked to be unscheduleable
Run same at 9.1 again with unsched.
Note any differences in any section.
}
...
Similar to section Unscheduleable Generators but set wind to random normal with s.d. taken from paper.
Try again with double and half penetration of wind, see how the range of p(acc) changes. Try 'box plot' ( the thing with mean, min, max, \& percentiles)
