...
This option first generates contingencies based upon the probabilistic
change during the deliver period. It then combines the contingencies
...
changes, then the contingencies not combines with a base-case. Next is
the first base case in the input followed by that base case combined
with each contingency given before in turn. This is repeated for all
...
This takes base cases or contingencies and simulates them in turn using
...
performed these ranged from: quick checks of individual functions;
...
if there is a mismatch it will be cancel out during calculation by the
...
these cases is taken to be of a fixed value and the rest of the
...
The code for this is given in \cite{appedix XXX}.
...
occurring such as CROW failures \cite{see XXX the Monte Carlo description in
...
part of other Ph.D. theses \cite{XXX - chan, dale?, berry?} it was
...
The table \ref{XXX} shows the summary of the results and forms a quick
...
optimisation can be directed to the most important part.

As they are very different code paths the different options given to
main.py produce completely different profile outputs. Because of this
they were tested separately. On one run of the program only 23 functions
actually spend more than 0.01 seconds executing. It seems obvious that
the focus of the optimisation should focus one just those 23.
...
could be spurious. This is discussed in section \ref{xxx 7.5} as point 7.
...
The number of simulations we determined in the simplest fashion that
