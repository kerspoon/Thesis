...
A key publication on this subject is the 'Renewables Network Impact Study' by the Carbon Trust \& DTI \cite{Trust2005}, it states that:
...
Geothermal power is driven by a steam turbine generating power by heat taken from the earths core. It provides a significant proportion of power in Iceland, though many place would not have the necessary conditions. The UK does have a capability for Geothermal power but it is not likely to become significant.
...
% ================================================================== %
\chapter{Power System Simulation}
% ================================================================== %

This section introduces the different options for modelling power systems: details the parts of a dynamic simulator; discusses how to model wind power before highlighting the changes made to the load-flow program PSAT to make it suitable for later experiments.
...
Before completion the project was dropped as it became apparent that it would not be possible to complete in the required time-frame. Research into load-flow simulation indicated that it would be possible to get initial results without the detail, and time, of a full dynamic simulation. Hence PSAT, a Matlab toolbox, was used instead.
...
Even more specialised is the GPU, or graphics card, is is made for computer games. It excels at just the task required for power system simulation. Only recently have developers been able to use graphics cards for general purpose calculations (this is known as \emph{GPGPU} \cite{Che2008}). This would require a major rethink due to the idiosyncrasies of the device but the advantage should be large.
...
Work was done to modify PSAT to allow a load-flow to more accurately deal with faulted systems by using this special form of distributed slack bus.
...
We can also gather information by looking at the difference between each of the points in disagreement and the threshold. This givens an indication of how badly the SAS is when errors do occur. If a very insecure system, say with a security level of 10 is reported as a failure by a SAS then this is a larger problem than one with a security level of 49. A system with a security level of 10 is very likely to have some problem during its operation. This is by definition. If we allow such a system to run in that state then we are open to the financial cost of whatever stability or limits violation will occur. We can sum up the difference between each operating condition in disagreement and the threshold to come up with a value for the severity of error in the SAS. It should be noted that this number is not meant to represent anything in reality but is simply an indication of how well the SAS is for those operating conditions tested on that specific system. It can however be compared to another SAS run over the same operating conditions on the same system, to see which is best.
...
The security threshold is an important factor in the success of the proposed scheme. More work needs to be done tn determine the actual tolerable level of security as it is defined by the method above.
...
\subsection{The IEEE Reliability Test System 96\label{lbl_RTS}}

\todo{replace one area with three area}

The IEEE Reliability Test System 96 (IEEE-RTS) \cite{Grigg1999} is a sample power system with a thorough set of data for operation, emissions, and reliability. It was for this reason that it was chosen as the test system for this work. Only area A was used, which is composed of 32 generating units, 24 busbars, 38 lines, 17 loads and two voltage levels.
...
In addition to new data the system has increased in size to incorporate three almost identical areas.

There have been many changes to power systems in the last 14 years and it would have been good if the system could be updated to incorporate these changes. The largest of the changes is the increase in penetration of renewable generation. As it stands the IEEE-RTS includes no renewable generators.
...
% ================================================================== %
\subsection{Monte Carlo Sampler Program}\label{sec_mc_results}


\todo{add wind talk? (maybe here maybe later)}
\todo{a lot of this can be moved into the discussion but I'm not sure if it should.}
...
The second stage takes each scenario through another round of Monte Carlo sampling; this time it samples for unplanned changes. Load forecast error is consider but only in the most basic form, a normally distributed random number with mean 1 and s.d. 0.05 is multiplied by the forecast given in stage one.
...
\image{samplerflow}{Flow Diagram for Stage 1 Sampling}{samplerflow.png}
...
The program itself was written mostly in Python and is available on-line at \url{http://github.com/kerspoon/kiribati} it uses a load-flow program as the core power system simulator. Owing to it's speed CPF  was selected as the simulator for this program.

\todo{cite and describe cpf}

It has a number of functions which all take input from the command line. This allows it to be combined together easily and intermediate parts written out to file.
...
This option outputs a list of base-cases to the command line. The base cases are generated using Monte Carlo Sampling as described in [chapter XXX], taking in to account MTTF, MTTR, CROW failures, and Load-level.

The parameter is the number of unique base cases to output. For instance if two base cases had a bus bar level of 0.55 and only G23 on outage then it would output a single line.
...
The third and fourth detail the results of simulation which are not defined until simulate is run on the file.

The fifth column is the load level. This number is the percentage of the peak that all loads will be scaled to. For instance if the load for a particular bus was 6.2 before the sample was applied and the load-level was 0.5 the resulting bus level would be 3.1, this is applied equally across the busses.

The rest of the columns list the names of components that are not to be included in the simulation. In base-cases this represents maintenance or previously fail components that are not available during the delivery period. In contingencies it represents components that will fail during the current delivery period.
...
\begin{verbatim}
# generate 1000 unique contengencies
# using the base cases in base.csv and save to file.
python main.py contingency 1000 < base.csv > contingency.csv
\end{verbatim}

This option first generates contengencies based upon the probabilistic
change during the deliver period. It then combines the contengencies
with the base cases given on the standard input.

For example if there we only two base cases given:
...
The output here is in sections, first is the raw base base with no
changes, then the contengencies not combines with a base-case. Next is
the first base case in the input followed by that base case combined
with each contengency given before in turn. This is repeated for all
base cases. This means there will be:

x = (B+1)*(C+1)

samples in the output file, and hence that many simulations to perform.

Note that the same samples are used for each base case. This does not
neccessarily represent reality but as long as there are enough samples
...
41,616 contengencies.

Like the \texttt{contengency} option it combines the set of
contengencies with each base case ready for simulation.
...
# generate all single component failures
# using the base cases in base.csv and save to file.
python main.py simulate < in.csv > results.csv
\end{verbatim}

This takes base cases or contengencies and simluates them in turn using
...
As the file is read in and simulated one line at a time stopping the
program will result in a certain portion of the simulations correctly
run. This was useful to quickly check results before simulating a large
run.

It is possible to specify all steps in one go, elimating intermediate
files. Unfortunatly this gives no feedback as to progress, it also means
...
contengencies combined with base cases and condenses them down. It
doesn't matter whether the contengencies were generated using monte
carlo or n-x.
...
Test prints out the results of simulating a number of interesting
base-cases that stress the program in different ways. It also prints the
file that is used as an input for CPF. A file that if normally never
seen. This is to ensure that it is being produced properly for a number
of edge cases. The cases selected also make sure that the simulator is
tested on a number of edge cases as well.

Finally there is a command to delete all files from previous runs so
that the working directory is clear to start again.
...
To ensure the proper operation of the program there were various test
performed these ranged from: quick checks of individial functions;
checking simulation output with hand worked examples; to an analysis of
how many samples were actually needed to ensure that it was not the
limiting factor.

\subsubsection{Unit Tests}

The simplest of these checks are the small unit tests. These are best
described by David Thomas et al. in Pragmatic Unit Testing:
...
While the program was being developed unit-test were programmed to run
every time the code was, that was if any change I had made broke
anything I would find out straight away.

The extract of code below makes sure that the individual function
\texttt{quantised\_01} does what I expect it to on a few hand worked
examples.
...
These functions althouth not individually useful in calcluating anything
...
boundries of modules as seen externally. This makes sure that the
...
then no further testing of the internals should be neccessary. One of
...
easily be hand worked so for example the first hour of the year (01/Jan
...
if there is a masmatch it will be cancel out during calcluation by the
slack-bus. If there are significant mismatches, such as from removing
certain generatios as is needed for this PhD it can lead to problems.
The simulation may end up testing the ability of the slack-bus to cope
with changing power level rather than the entire system. For this reason
...
generator was to hit a limit it is simple set at full power and the rest
of the power distributed among the other generators.

Certain generators are not suitable for varyation, these are the ones
where power output is not controlable, for example wind turbines. In
these cases is taken to to be of a fixed value and the rest of the
generators must make up the difference.

Sometimes the power requirement is too high to be met by the available
generation. This is a problem of adaquacy and as such the particular
...
problem with the simulation but accuratly represents a problem in the
system.

The code for this is givin in \cite{appedix XXX}.
...
limit of less than 1.5 or the initial powers of the generator dofferred
...
\begin{quote}
Integration testing shows that the major subsystems that make up the project work and play well with each other. [XXX the pragmatic programmer]
\end{quote}
...
500 tails. If the monte carlo sampler was working it should give results
similar to this.
...
occuring such as CROW failures {[}see XXX the monte carlo description in
this chapter{]}. These effects should be dwarfed by the overall
probabilities and as such were ignored.

\paragraph{Theoretical Results}

\todo{take from old thesis}

\paragraph{Monte carlo Results and comparason}
...
part of other PhD thesis {[}XXX - chan, dale?, berry?{]} it was
important to ensure it worked in the content of what was done for this
work. There was also addition code that took a sample and created the
input file, properly modified, to represent it.
...
that the correct components have ben removed and that the load and
...
busses and branches to reflect that change.

Once the intermediate file were checked the results of the simulation
...
The table {[}XXX{]} shows the summary of the results and forms a quick
reference for future checking. Making sure this works and can be
automated means that the program itself can be changed easily for
reasons of speed or clarity; something discussed in the next section.

\todo{simulator testing table}
...
simulate sample that were different from each other, this saved many
...
optimisiations that seem logical can slow down execution due to cache
sizing issues and compiler optimisiation. The key tool in reducing the
...
any python run it creates a file contaning profile information. This can
...
see which part of the program is the slowest so the focus of
optimisiation can be directed to the most important part.

As they are very different code paths the differnt options given to
main.py produce completely differnt profile outputs. Because of this
they were tested separatly. On one run of the program only 23 functions
actually spend more than 0.01 seconds executing. It seems obvious that
the focus of the optimisiation should focus one just those 23.

\todo{table of longest calls}

\todo{ call graph.} 
\todo{ graph of long tail of function times.}
...
could be spurious. This is dicussed in section {[}7.5{]} as point 7.
Although not so problematic it is possible to do too many simulations in
that other factors may have to be reduced thus decreasing overall
accuracy.

The number of simulations we determined in the simplest fashon that
works well. If the result is stable in repeated runs then the number of
samples is not the limiting factor and you can be reasonably sure that
the result will not get more accurate simply by running more
simulations.
...
This was done repeatedly starting with 10 unique samples going up an
order of magnitude each time until 100,000 uniques samples. These
...
The results for one such test are shown below. You can see that the
...
\todo{graph of results on 10, 100, 1000 etc.}
\todo{graph of results on 10, 100, 1000 etc. for a few base-cases}

\section{Using the program}

Here is the shell script that does each stage of simulation in order to
produce the final analysis. First it removes old file, then generates
base cases and simulate them. Then it generates contingencies and
...
\chapter{Results}
% ================================================================== %
This chapter presents the implementation details and results of the Monte Carlo Sampling program.
...
\subsection{Comparing Theoretical with Monte Carlo Results}
...
Analysis of the final body of work occurs in this chapter. This includes detailing the program used for simulation as well as the testing of the Monte Carlo sampling program that is central to the work. It is shown that shown that it accurately represents the underling system.
